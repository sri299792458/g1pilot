...
- ROS 2 Humble is already installed at `/opt/ros/humble`; set `ROS_DISTRO=humble` when selecting the setup scripts.
...
```bash
cd /path/to/g1pilot
sudo scripts/setup_system_deps.sh --full
```
...
- `~/g1pilot_humble_ws/src/g1pilot` as a symlink to the current repo.
- `~/g1pilot_humble_ws/src/astroviz_interfaces`.
- `~/g1pilot_humble_ws/external/unitree_sdk2_python`.
- `~/g1pilot_humble_ws/.venv` with the Python packages needed by this repo.
- `~/g1pilot_humble_ws/deps` with the Docker-equivalent user-space source dependencies.
- `~/g1pilot_humble_ws/env_humble.sh` for future terminals.

The setup preserves these dependency and build constraints:
...
This builds or installs, under `~/g1pilot_humble_ws/deps`, the Docker-equivalent heavy pieces:
...
```bash
JOBS=8 scripts/setup_user_workspace.sh
```

Useful troubleshooting options:

```bash
scripts/setup_user_workspace.sh --skip-opensot
scripts/setup_user_workspace.sh --skip-livox
scripts/setup_user_workspace.sh --skip-realsense-python
scripts/setup_user_workspace.sh --skip-teleimager-client
```
...
```bash
source ~/g1pilot_humble_ws/env_humble.sh
cd ~/g1pilot_humble_ws
...
```bash
source scripts/source_g1.sh real <interface>
```
...
The lab G1 reported `mode_machine=5` on `enp134s0`, but its waist roll/pitch joints are physically locked and waist yaw remains free. Set the Unitree-side waist configuration to locked-waist / 1-DOF mode if available, then use a locked-waist URDF in this stack: yaw stays active, roll/pitch must be fixed and uncommanded.
